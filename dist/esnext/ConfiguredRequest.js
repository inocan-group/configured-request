...
import { DynamicStateLocation } from "./index";
...
export class ConfiguredRequest {
    ...
    async request(props, runTimeOptions = {}) {
        ...
        if (result.status >= 300) {
            throw new RequestError(`The API endpoint [ ${this._method.toUpperCase()} ${url} ] failed with a ${result.status} / ${result.statusText}} error code.`, String(result.status), result.status);
        }
     ...
     */
    requestInfo(props) {
        const qp = Object.assign(Object.assign({}, this.getDynamics(DynamicStateLocation.queryParameter, props)), this._qp);
        const url = this._url
            .split("{")
            .map(i => i.replace(/(.*})/, ""))
            .join("");
    ...
    setUrl(url) {
        ...
        if (url.includes("{")) {
            ...
            parts.slice(1).forEach(part => {
                ...
                if (endDelimiter === -1) {
                    throw new RequestError(`The structure of the URL parameter is invalid. URL's which have dynamic parameters as part of the URL must be delimited by a openning and closing curly brackets. The current URL appears to have a mismatch of openning and closing brackets. The URL is: ${url}`, "invalid-url");
                }
    ...
    async makeRequest(body, url, options) {
        switch (this._method) {
            ...
            case "put":
                return axios.put(url, this._body, options);
            case "post":
                return axios.put(url, this._body, options);
            case "delete":
                return axios.delete(url, options);
            case "patch":
                return axios.patch(url, this._body, options);
        }
    }
    /**
